$(document).ready(function(){
    ...
    $('.nav-link').on('click', function() {
        var scrollAnchor = $(this).attr('href'),
            scrollPoint  = $('section[id="'+scrollAnchor+'"]').offset().top +20;
	...
	$(function() {
		...
		var scContent = $(".scale-out");
		$(window).scroll(function() {    
			var scroll = $(window).scrollTop();
			for (var i = 0; i < scContent.length; i++) {
			if ( $(window).scrollTop() > ($('.scale-out').eq(i).offset().top-elementOffset)) {
				scContent.eq(i).addClass('fade-content');
				scContent.eq(i).css("opacity","1");
			}}
    ...
    $('.scroll').each(function(i) {
            if ($(this).position().top <= scrollDistance ) {
                    $(".nav-link.add-active").removeClass('add-active');
                    $(".nav-link").eq(i).addClass('add-active');
            }
    });
}).scroll();
